...
su="sudo "
getConfig() { grep "$1" "../../../Output/BuildConfig.ini" | cut -d "=" -f 2; }  #读取配置文件
echo "强制加密：$(getConfig 'IS_ENCRYPTION')    AVB校验：$(getConfig 'IS_AVB_VERIFY')"
if [ "$(getConfig 'IS_AVB_VERIFY')" = 'FALSE' ]; then
	if [ -f "split_img/boot.img-dtb" ]; then
		sed -i 's/\x2C\x61\x76\x62/\x00\x00\x00\x00/g' split_img/boot.img-dtb
	fi

	sed -i 's/\x2C\x61\x76\x62/\x00\x00\x00\x00/g' split_img/boot.img-kernel
	sed -i 's/\x2C\x76\x65\x72\x69\x66\x79/\x00\x00\x00\x00\x00\x00\x00/g' split_img/boot.img-kernel
	#sed -i '/buildvariant=user/s/$/ androidboot.verifiedbootstate=green androidboot.selinux=permissive/g' split_img/boot.img-cmdline
fi

${su} rm -rf ramdisk/verity_key 2>/dev/null
